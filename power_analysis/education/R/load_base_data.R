...
# earliest year of obs in the data is earliest yob for somebody who could have received a height/edu/weight measure
...
data$alive = data$alive[, 5:19]

# store the aoc based on alive
data$alive = apply(data$alive, 1, function(x) {which(x == -99)[1]})
data$alive[is.na(data$alive)] = 15
