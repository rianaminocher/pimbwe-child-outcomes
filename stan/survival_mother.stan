...
int unmarried [N, A]; // whether mother is unmarried
int married_not_to_father [N, A]; // whether mother is married to not father
...
vector [A] a_age_raw [7];
real <lower = 0, upper = 1> a_age_kappa [7];
real <lower = 0> a_age_tau [7];
real <lower = 0> a_age_delta [7];
  ...
  // when a == 1, we cannot condition on survival in (a-1)
    if (alive[n, 1] != -99) {
      if (skip[n, 1] == 0) {
        ...
        a_age[2, 1] * male[n] + 
        a_age[3, 1] * twin[n] + 
        a_age[4, 1] * fatherdead[n, 1] +
        a_age[5, 1] * motherdead[n, 1] +
        a_age[6, 1] * unmarried[n, 1] +
        a_age[7, 1] * married_not_to_father[n, 1]);
      }
  }
        ...
        if (alive[n, a] != -99) { // if survival is not unknown
          if (alive[n, a-1] == 1) { // if survived previous year
            if (skip[n, a] == 0) {
                  ...
                  a_age[2, a] * male[n] + 
                  a_age[3, a] * twin[n] + 
                  a_age[4, a] * fatherdead[n, a] +
                  a_age[5, a] * motherdead[n, a] +
                  a_age[6, a] * unmarried[n, a] +
                  a_age[7, a] * married_not_to_father[n, a]);
...
generated quantities {
  ...
  vector [A] p_mother_married_not_to_father [2];

  vector [A] cumulative_p_base [2];
  vector [A] cumulative_p_mother_dead [2];
  vector [A] cumulative_p_mother_unmarried [2];
  vector [A] cumulative_p_mother_married_not_to_father [2];
  ...
  for (a in 1:A) {
    ...
    for (i in 1:2) { // male or female
      ...
      p_mother_married_not_to_father[i, a] = inv_logit(alpha + 
                                                       a_bo[1] + 
                                                       a_year[61 - (a - 1)] +
                                                       a_age[1, a] +
                                                       a_age[2, a] * (i-1) +
  ...
  for (i in 1:2) {
    ...
    cumulative_p_mother_dead[i] = cumulative_product(p_mother_dead[i]);
    cumulative_p_mother_unmarried[i] = cumulative_product(p_mother_unmarried[i]);
    cumulative_p_mother_married_not_to_father[i] = cumulative_product(p_mother_married_not_to_father[i]);
